...
export const Character = () => {
  ...
  const sendCharacter = async () => {
    const res = await fetch(
      "https://h3nckvn8-8000.asse.devtunnels.ms/api/user/insert/",
      {
        method: "POST",
        headers: {
  ...
  return (
    <CharacterLayout>
      {scene === "username" && (
        <UserNameScene next={goOekaki} setUserName={setUserName} />
      )}
      {scene === "oekaki" && (
        <OekakiScene next={goFinish} setCharacter={setCharacter} />
      )}
